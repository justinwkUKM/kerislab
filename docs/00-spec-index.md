...
5. Scan core: create scan, reserve credit, run worker, stream events, persist findings and evidence.
6. Autonomous blackbox MVP: guarded agent loop, Playwright tools, safe HTTP tools, Mission Control UI.
7. Approval gates: approval requests, pause/resume, operator instructions, audit trail.
8. Whitebox MVP: repository ingestion, Semgrep, dependency scanning, secret scanning, code-aware findings.
9. Reporting: JSON, Markdown, PDF, SARIF, retest workflow.
10. Scale and hardening: worker isolation, object storage, Kubernetes, budget enforcement.
...
- Authentication: Google OAuth plus enterprise SSO from MVP.
